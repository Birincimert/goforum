...
jquery__WEBPACK_IMPORTED_MODULE_0___default().extend(true, (jquery__WEBPACK_IMPORTED_MODULE_0___default().summernote).lang, {
  'en-US': {
    ...
    image: {
      ...
      dragImageHere: 'Drag img or text here',
      dropImage: 'Drop img or Text',
    ...
    */
    ...
    value: function insertImage(src, param) {
      ...
      return createImage(src, param).then(function ($image) {
        ...
        _this3.context.triggerEvent('img.upload.error', e);
      });
    }
    ...
    value: function insertImagesAsDataURL(files) {
      ...
      external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default().each(files, function (idx, file) {
        ...
        if (_this4.options.maximumImageFileSize && _this4.options.maximumImageFileSize < file.size) {
          _this4.context.triggerEvent('img.upload.error', _this4.lang.image.maximumFileSizeError);
        } else {
          readFileAsDataURL(file).then(function (dataURL) {
            return _this4.insertImage(dataURL, filename);
          }).fail(function () {
            _this4.context.triggerEvent('img.upload.error');
          });
        }
      });
    }
    ...
    value: function insertImagesOrCallback(files) {
      ...
      if (callbacks.onImageUpload) {
        this.context.triggerEvent('img.upload', files);
  ...
  return Handle_createClass(Handle, [{
    ...
    value: function update(target, event) {
      ...
      if (isImage) {
        ...
        }).data('target', $image); // save current img element.
...
var ImageDialog = /*#__PURE__*/function () {
  ...
  return ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group note-group-select-from-files">', '<label for="note-dialog-img-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-img-file-' + this.options.id + '" class="note-img-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="' + this.options.acceptImageFileTypes + '" multiple="multiple"/>', imageLimitation, '</div>', '<div class="form-group note-group-img-url">', '<label for="note-dialog-img-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-img-url-' + this.options.id + '" class="note-img-url form-control note-form-control note-input" type="text"/>', '</div>'].join('');
      var buttonClass = 'btn btn-primary note-btn note-btn-primary note-img-btn';
      ...
      this.showImageDialog().then(function (data) {
        ...
        if (typeof data === 'string') {
          ...
          if (_this.options.callbacks.onImageLinkInsert) {
            _this.context.triggerEvent('img.link.insert', data);
          } else {
    ...
    value: function showImageDialog() {
      var _this2 = this;
      return external_root_jQuery_commonjs_jquery_commonjs2_jquery_amd_jquery_default().Deferred(function (deferred) {
        var $imageInput = _this2.$dialog.find('.note-img-input');
        var $imageUrl = _this2.$dialog.find('.note-img-url');
        var $imageBtn = _this2.$dialog.find('.note-img-btn');
        _this2.ui.onDialogShown(_this2.$dialog, function () {
          _this2.context.triggerEvent('dialog.shown');
...
var imageDialog = function imageDialog(opt) {
  var body = '<div class="note-form-group note-group-select-from-files">' + '<label for="note-dialog-img-file-' + opt.id + '" class="note-form-label">' + opt.lang.image.selectFromFiles + '</label>' + '<input id="note-dialog-img-file-' + opt.id + '" class="note-note-img-input note-input" type="file" name="files" accept="img/*" multiple="multiple"/>' + opt.imageLimitation + '</div>' + '<div class="note-form-group">' + '<label for="note-dialog-img-url-' + opt.id + '" class="note-form-label">' + opt.lang.image.url + '</label>' + '<input id="note-dialog-img-url-' + opt.id + '" class="note-img-url note-input" type="text"/>' + '</div>';
  var footer = ['<button href="#" type="button" class="note-btn note-btn-primary note-btn-large note-img-btn disabled" disabled>', opt.lang.image.insert, '</button>'].join('');
  return dialog({
