package controllers

import (
	"fmt"
	"goforum/admin/helpers"
	"goforum/admin/models"
	sitemodels "goforum/site/models"
	"html/template"
	"io"
	"net/http"
	"os"
	"strconv"

	"github.com/gorilla/sessions"
	"github.com/gosimple/slug"
	"github.com/julienschmidt/httprouter"
)

//Temanın ana sayfası için

type Dashboard struct {
	Store *sessions.CookieStore
}

func (dashboard Dashboard) Index(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	view, err := template.New("index").Funcs(template.FuncMap{
		"getCategory": func(categoryID int) string {
			return models.Category{}.Get(categoryID).Title
		},
	}).ParseFiles(helpers.Include("/dashboard/list")...)

	if err != nil {
		fmt.Println(err)
		return
	}
	data := make(map[string]interface{})
	data["Posts"] = models.Post{}.GetAll()
	data["Alert"] = helpers.GetAlert(w, r, dashboard.Store)

	view.ExecuteTemplate(w, "index", data)
}

func (dashboard Dashboard) NewItem(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	view, err := template.ParseFiles(helpers.Include("dashboard/add")...)
	if err != nil {
		fmt.Println(err)
		return
	}

	//Kategorileri çekip data olarak göndericez ilgili yere
	data := make(map[string]interface{})
	data["Categories"] = models.Category{}.GetAll()

	view.ExecuteTemplate(w, "index", data)
}

func (dashboard Dashboard) Add(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	title := r.FormValue("blog-title")
	slug := slug.Make(title)
	description := r.FormValue("blog-desc")
	categoryID, _ := strconv.Atoi(r.FormValue("blog-category"))
	content := r.FormValue("blog-content")

	//Upload (opsiyonel)
	r.ParseMultipartForm(10 << 20)
	var pictureURL string
	if file, header, err := r.FormFile("blog-picture"); err == nil && header != nil && header.Filename != "" {
		// uploads klasörü yoksa oluştur
		if _, statErr := os.Stat("uploads"); os.IsNotExist(statErr) {
			_ = os.MkdirAll("uploads", 0755)
		}
		f, openErr := os.OpenFile("uploads/"+header.Filename, os.O_WRONLY|os.O_CREATE, 0666)
		if openErr == nil {
			_, _ = io.Copy(f, file)
			pictureURL = "uploads/" + header.Filename
		}
	}

	// Görsel yüklenmediyse varsayılan ana sayfa görselini kullan
	if pictureURL == "" {
		pictureURL = "uploads/mainpage.jpg"
	}

	models.Post{
		Title:       title,
		Slug:        slug,
		Description: description,
		CategoryID:  categoryID,
		Content:     content,
		Picture_url: pictureURL,
	}.Add()
	helpers.SetAlert(w, r, "Kayıt başarıyla eklendi", dashboard.Store)
	http.Redirect(w, r, "/admin", http.StatusSeeOther)
}

func (dashboard Dashboard) Delete(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	post := models.Post{}.Get(params.ByName("id"))
	// İlgili tüm yorumları da temizle
	_ = sitemodels.Comment{}.DeleteByPostID(post.ID)
	post.Delete()
	helpers.SetAlert(w, r, "Kayıt başarıyla silindi", dashboard.Store)
	http.Redirect(w, r, "/admin", http.StatusSeeOther)
}

func (dashboard Dashboard) Edit(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	view, err := template.ParseFiles(helpers.Include("dashboard/edit")...)
	if err != nil {
		fmt.Println(err)
		return
	}
	data := make(map[string]interface{})
	data["Post"] = models.Post{}.Get(params.ByName("id"))
	data["Categories"] = models.Category{}.GetAll()

	view.ExecuteTemplate(w, "index", data)
}

func (dashboard Dashboard) Update(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	if !helpers.CheckUser(w, r, dashboard.Store) {
		return
	}

	post := models.Post{}.Get(params.ByName("id"))

	title := r.FormValue("blog-title")
	slug := slug.Make(title)
	description := r.FormValue("blog-desc")
	categoryID, _ := strconv.Atoi(r.FormValue("blog-category"))
	content := r.FormValue("blog-content")
	is_selected := r.FormValue("is_selected")
	var picture_url string

	if is_selected == "1" {
		//is_selected degiscek

		//Upload
		r.ParseMultipartForm(10 << 20)
		file, header, err := r.FormFile("blog-picture")
		if err != nil {
			fmt.Println(err)
			return
		}
		f, err := os.OpenFile("uploads/"+header.Filename, os.O_WRONLY|os.O_CREATE, 0666)
		if err != nil {
			fmt.Println(err)
			return
		}
		_, err = io.Copy(f, file)
		//Upload end
		picture_url = "uploads/" + header.Filename
		os.Remove(post.Picture_url) //önceki görseli silme

	} else {
		//resim aynı kalcak
		picture_url = post.Picture_url
	}

	post.Updates(models.Post{
		Title:       title,
		Slug:        slug,
		Description: description,
		CategoryID:  categoryID,
		Content:     content,
		Picture_url: picture_url,
	})
	helpers.SetAlert(w, r, "Kayıt başarıyla güncellendi", dashboard.Store)
	http.Redirect(w, r, "/admin/edit/"+params.ByName("id"), http.StatusSeeOther)
}
